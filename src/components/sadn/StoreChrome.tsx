...
export function StoreChrome({
  ...
  return (
    <div
      data-screen-shell
      className="relative mx-auto flex min-h-dvh w-full max-w-[430px] lg:max-w-7xl flex-col border-x border-sadn-plum-100 lg:border-x-0 bg-sadn-canvas transition-all"
      data-add-context
    >
      {banner.visible && (
        <AnnouncementBanner textEn={banner.textEn} textAr={banner.textAr} />
      )}
      <TopBar />
      ...
      <div ref={screenRef} className="flex-1 w-full" data-children>
        {/* Server-rendered page content — cloned with the add/open handlers */}
        {childrenWithHandlers(children, ctx)}
      </div>

      <div className={path.startsWith('/product/') ? 'hidden lg:block' : ''}>
        <SadnFooter config={config} />
      </div>

      {/* Bottom Nav: Mobile only (hidden on desktop where TopBar takes full nav duties) */}
